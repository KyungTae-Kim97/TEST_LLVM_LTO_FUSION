        ...
        SecurityException& /*exception*/)
{
    ...
    uint32_t is_thunk_flag = 0;

    if (total_len >= 24) {
        memcpy(&is_thunk_flag, payload.data, 4);
    }

    if (is_thunk_flag == 0xDEADBEEF) {
        try {
            uint32_t real_size_meta;
            uint64_t func_ptr;
            uint64_t raw_data_ptr;

            memcpy(&real_size_meta, payload.data + 4, 4);
            memcpy(&func_ptr, payload.data + 8, 8);
            memcpy(&raw_data_ptr, payload.data + 16, 8);
