...
### 🛡️ Hardware-Accelerated Inline Crypto Engine
...
* **`AESGCMGMAC_Transform.cpp`**
  The core modification within the native eProsima Fast DDS security plugin. It acts as a **Dynamic Fast-Path Router** by inspecting incoming payloads for a designated Thunk flag (`0xDEADBEEF`). When detected, it bypasses the standard OpenSSL intermediate buffers (Slow-Path). Instead, it parses the injected metadata (raw data pointer, function pointer) and directly orchestrates the LTO fusion engine (`fuse_init`, inline memcpy execution, and `fuse_finalize_enc`). This seamlessly integrates zero-copy-like SIMD acceleration into the payload body while perfectly preserving the standard RTPS `SecureDataHeader` and `SecureDataTag` compliance.
...
* **Zero Call-Stack Overhead:** Eradicated the deep function call chains of standard cryptographic libraries via `__attribute__((always_inline))` and direct intrinsic mapping.
* **SIMD Parallelism:** Leveraged 128-bit NEON registers (`uint8x16_t`) to execute 8 AES block transformations concurrently, saturating the CPU's vector ALU.
* **Memory Safety & Efficiency:** Ensured secure, in-place cryptographic transformations directly within the RTPS payload buffers, preventing memory leaks and reducing L3 cache misses.