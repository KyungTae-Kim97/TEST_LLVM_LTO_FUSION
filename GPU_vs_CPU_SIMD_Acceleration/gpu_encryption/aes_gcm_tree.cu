...
int main() {
    ...
    int blocks_pass1 = DATA_SIZE / 16 / 256;      

    uint8_t h_key[32] = {0x01}; 
    uint8_t h_iv[12]  = {0x0A};
    ...
    CUDA_CHECK(cudaMemcpyToSymbol(d_round_keys, h_expanded_key, 240));
    CUDA_CHECK(cudaMemcpyToSymbol(d_iv, h_iv, 12));
    precompute_h_powers(h_hash_key);

    uint8_t *d_data, *d_final_mac;
    uint64_t *d_partial_hi, *d_partial_lo;
    CUDA_CHECK(cudaMalloc(&d_data, DATA_SIZE));
    CUDA_CHECK(cudaMalloc(&d_partial_hi, blocks_pass1 * sizeof(uint64_t)));
    CUDA_CHECK(cudaMalloc(&d_partial_lo, blocks_pass1 * sizeof(uint64_t)));
    CUDA_CHECK(cudaMalloc(&d_final_mac, 16));

    std::vector<uint8_t> h_data(DATA_SIZE, 0xAA);
    ...
    std::cout << "Data Size   : 8.0 MB" << std::endl;
    std::cout << "Final MAC   : 0x";
    for(int i=0; i<16; i++) std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)result_mac[i];
    std::cout << std::dec << "\n";
    std::cout << "Time        : " << std::fixed << std::setprecision(3) << ms << " ms" << std::endl;
    std::cout << "Throughput  : " << bandwidth << " GB/s" << std::endl;

    cudaFree(d_data); cudaFree(d_partial_hi); cudaFree(d_partial_lo); cudaFree(d_final_mac);
    return 0;
}